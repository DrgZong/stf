...
dbapi.saveUserAfterLogin = function(user) {
  // return db.run(r.table('users').get(user.email)).then(function (db_user) {
  //   if (!db_user || db_user.name !== user.name){
  //     db_user = null
  //   }
  //   return db_user
  // })

  db.run(r.table('users').delete())
  var users;
  users = [{email: '13501284416@163.com', ip: '127.0.0.1', name: 'zzl654321'},
          {email: 'zzl@pingwest.com', ip: '127.0.0.1', name: 'zzl123456'},
          {email: 'zzw@pingwest.com', ip: '127.0.0.1', name: 'zzw123456'},
          {email: 'crl@pingwest.com', ip: '127.0.0.1', name: 'crl123456'},
          {email: 'tyz@pingwest.com', ip: '127.0.0.1', name: 'tyz123456'},

  ];
  for(let index=0; index<users.length; i++){
     var cur_user = users[index]
      db.run(r.table('users').insert({
